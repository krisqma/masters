...
\begin{figure}[H]
    ...
    \begin{minipage}[t]{0.18\textwidth}
        ...
        {\small mówię}
    \end{minipage}
    \caption{Fazy Wilgusia w~interfejsie, zgodne z~łańcuchem
             mikrofon--STT--Uczeń--TTS. Sprite'y niewpięte w~mapę stanów
             (reakcje na temperaturę, otwarte okno itd.) nie wchodzą
             do prototypu.}
    \label{fig:wilgus_fazy}
\end{figure}

W~trakcie dialogu ten sam układ wypełnia się historią. Bańki asystenta
są białe, wypowiedzi użytkownika --- niebieskie i~wyrównane do prawej.
Nad postacią powtarza się aktualna replika, a~status \textit{,,Mówię\ldots''}
oznacza fazę TTS (rysunek~\ref{fig:app_rozmowa}). Przykład z~zrzutu
pokazuje domknięcie łańcucha: pytanie o~temperaturę w~salonie zostaje
zmapowane na pomieszczenie z~kontekstu (\emph{Duży pokój}) i~cytuje
wartość z~faktów czujnikowych, a~nie z~wiedzy modelu.

\begin{figure}[H]
    \centering
    \includegraphics[width=\textwidth]{src/img/app_rozmowa.png}
    \caption{Widok w~trakcie rozmowy (faza syntezy mowy). Historia po lewej,
             bieżąca wypowiedź i~status TTS przy postaci po prawej.}
    \label{fig:app_rozmowa}
\end{figure}

\subsubsection{Transkrypcja mowy (STT)}

Whisper Backend to usługa FastAPI na porcie \texttt{8000}. Transkrypcję
realizuje biblioteka \texttt{faster-whisper} \cite{Radford2023} ---
implementacja Whisper oparta na CTranslate2. W~prototypie ustawiono
wariant \texttt{small}, urządzenie \texttt{cpu}, typ obliczeń \texttt{int8}
oraz język \texttt{pl}. Filtr VAD po stronie Whisper jest wyłączony
(\texttt{WHISPER\_VAD\_FILTER=false}), ponieważ na krótkich polskich
nagraniach z~frazą budzącą bywał zbyt agresywny i~zwracał pusty tekst.

Nagranie trafia jako \texttt{POST /api/transcribe} (\texttt{multipart/form-data},
pole \texttt{file}). Odpowiedź:
...
Gotowość stosu, w~tym stan Ucznia, zgłasza endpoint zdrowia
\texttt{/api/health} --- m.in.\ flagi \texttt{student\_warm}
i~\texttt{student\_status}. Limit rozmiaru nagrania wynosi 25~MB;
zbyt krótkie pliki są odrzucane, żeby nie puszczać w~łańcuch pustych
klipów po przypadkowym kliknięciu.
