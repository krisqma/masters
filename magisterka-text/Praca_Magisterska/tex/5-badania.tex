...
Czas, po którym wczasowicz otrzyma odpowiedź, zależy także od jej długości: modele i~warianty
charakteryzują się różną zwięzłością Rysunek~\ref{fig:tok_s_variant} pokazuje
więc szybkość generacji [znaki/s], liczoną per rekord jako
$\mathrm{answer\_chars}/(\mathrm{generation\_ms}/1000)$. Modele kwantyzowane (doskonale widać to na przykaładzie modeli Gemma Q2\_K)
wykazują zbliżoną lub wyższą przepustowość znaków niż bazowy wariant Q4
(ok.\ 10~zn/s u~\texttt{gemma3-4b}), a~mimo to czas odpowiedzi jest dłuższy, co widać na rysunku~\ref{fig:gen_ms_variant}. Wynika to z utraty zwięzłości, omówionej w~sekcji~\ref{subsubsec:anomalie}.
