...
się bliżej drugiemu kluczowemu aspektowi asystenta głosowego --- opóźnieniu
end-to-end (E2E). Benchmark zbierał już te pomiary (tabela~\ref{tab:benchmark_metrics}),
więc nie było konieczne przeprowadzanie nowych ewaluacji. Analizę rozpoczęto od
dopasowania rozkładu: w~protokole Optuny (zmienny prompt, cold start) czasy E2E
układają się w~krzywą Gaussa i~modelowane są jako
$\mathcal{N}(\hat\mu,\hat\sigma^2)$. Rysunek~\ref{fig:optuna_gauss_fit} pokazuje
histogramy E2E oraz dopasowaną gęstość per model.
...
\begin{figure}[H]
  ...
  \caption{Dopasowanie rozkładu normalnego do opóźnienia E2E.
           Histogram oraz gęstość $\mathcal{N}(\hat\mu,\hat\sigma^2)$ per model.}
  \label{fig:optuna_gauss_fit}
\end{figure}

Przyjęto lokalizację $\hat\mu$ (średnia E2E) oraz skalę $\hat\sigma$ (odchylenie
standardowe). Przedział $\hat\mu\pm\hat\sigma$ opisuje typowy zakres pomiarów przy
dobrym dopasowaniu normalnym, a~$\hat\mu\pm 2\hat\sigma$ --- szerszy zakres
zmienności między przebiegami. Są to charakterystyki robocze modelu Gaussa,
a~nie formalny test normalności. Parametry per model zestawiono w~tabeli~\ref{tab:optuna_e2e_gauss}.

\begin{table}[H]
  \centering
  \caption{Model Gaussa dla E2E w~protokole Optuny: lokalizacja~$\hat\mu$,
           skala~$\hat\sigma$, przedziały $\hat\mu\pm\hat\sigma$
           i~$\hat\mu\pm 2\hat\sigma$ oraz współczynnik zmienności
           $\mathrm{CV}=\hat\sigma/\hat\mu$.}
...
Modele różnią się przede wszystkim lokalizacją, a~nie profilem zmienności:
CV pozostaje bliskie $0{,}22$--$0{,}25$. Ranking średniego E2E to
\texttt{gemma2:2b} ($\hat\mu\approx 20{,}5$~s), następnie
\texttt{llama3.2:3b} ($\approx 26{,}8$~s), a~dalej para
\texttt{qwen2.5:3b} i~\texttt{gemma3:4b} ($\approx 29{,}4$--$30{,}4$~s).
Przedziały $\hat\mu\pm\hat\sigma$ obu Gemm stykają się lekko
(górna krawędź \texttt{gemma2:2b} $\approx 25{,}7$~s, dolna krawędź
\texttt{gemma3:4b} $\approx 22{,}8$~s), ale środki rozkładów są wyraźnie
rozdzielone. Llama i~Qwen leżą czasowo bliżej wolniejszej Gemmy niż szybszej.

Połączenie z~analizą jakościową zmienia jednak obraz rankingowy. Przy E2E
porównywalnym z~\texttt{gemma3:4b} modele \texttt{llama3.2:3b}
i~\texttt{qwen2.5:3b} mają wskaźnik halucynacji rzędu 16--21\% --- wielokrotnie
wyższy niż ok.~1{,}2\% u~\texttt{gemma3:4b} --- więc nie tworzą atrakcyjnego
kompromisu na tym etapie. Na froncie jakość--czas pozostają obie Gemmy:
\texttt{gemma2:2b} jest wyraźnie szybsza, lecz częściej halucynuje;
\texttt{gemma3:4b} jest wolniejsza o~ok.~10~s średniego E2E, lecz niemal nie
wprowadza faktów spoza kontekstu.

Kompromis Pareto (rysunek~\ref{fig:pareto_latency_hall}) formalizuje ten układ:
żaden model nie dominuje jednocześnie w~obu wymiarach.
\texttt{gemma2:2b} ma najniższe $\hat\mu$ E2E, lecz ok.~18\%~halucynacji;
\texttt{gemma3:4b} jest wolniejsza ($\hat\mu\approx 30{,}4$~s), lecz osiąga
...
wybór \texttt{gemma3:4b} jako kandydata do kolejnych etapów optymalizacji
opóźnienia. Rozsądny preset hiperparametrów wystarcza; Optuna nie wskazuje
silnie dominującej konfiguracji jakościowej.
