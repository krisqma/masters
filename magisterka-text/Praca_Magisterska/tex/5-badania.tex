...
Literatura zapowiadała, że obniżenie precyzji wag przyspiesza inferencję,
ponieważ dekodowanie tokenów jest ograniczone przepustowością pamięci
\cite{Kim2023SqueezeLLM}. Pierwsze porównanie wariantów GGUF na metryce E2E
tego nie potwierdzało. Modele przez wiele przebiegów nie wykazywały
czytelnego przyspieszenia przy przejściu z~Q8\_0 do Q4: warianty Q4\_0
i~Q4\_K\_M dla \texttt{gemma3-4b} były praktycznie nierozróżnialne.
Tabela~\ref{tab:e2e_wstepne} zestawia mediany modelu log-normalnego
$e^{\hat\mu}$ całkowitego czasu odpowiedzi ($n\approx 100$), bez
rozbicia na TTFT i~generację --- tak, jak wyglądało pierwsze spojrzenie
na wyniki.
...
Dla obu modeli Gemma Q4\_0 i~Q4\_K\_M różnią się o~ułamek sekundy.
Q8\_0 jest wyraźnie wolniejszy, lecz nie w~sposób, jakiego oczekiwano
po samej kwantyzacji dekodowania: różnica nie układa się proporcjonalnie
do precyzji wag. Wariant Q2\_K wypada gorzej niż Q4 mimo najmniejszego
pliku GGUF; anomalię tę omówiono w~sekcji~\ref{subsubsec:anomalie}.

Poszukiwanie przyczyny rozpoczęto od analizy literatury o~inferencji
transformerów oraz dokumentacji silnika Ollamy. Pope et al.\ \cite{Pope2022}
wprowadzili rozróżnienie dwóch faz: \emph{prefill} (równoległe
przetworzenie całego promptu i~zbudowanie pamięci kluczy i~wartości,
...
dekodowania, ograniczoną przepustowością pamięci \cite{Kim2023SqueezeLLM}.
...
ponowne liczenie prefillu, o~ile prefiks promptu pozostaje identyczny.
W~Ollamie model jest z~RAM usuwany po okresie bezczynności; parametr
\texttt{keep\_alive} (domyślnie 5~minut) steruje czasem rezydencji
\cite{OllamaAPI}. Silnik llama.cpp, na którym opiera się Ollama,
udostępnia analogiczny mechanizm ponownego użycia prefiksu KV
(\texttt{cache\_prompt}).

Z~tego wynika, że pierwszy protokół pomiarowy był de facto stanem
\emph{cold}: przy każdym zapytaniu --- lub po wyładowaniu modelu ---
urządzenie wykonywało pełny prefill od zera. Metryka E2E była więc
sumą długiego TTFT i~krótkiej generacji,
$\mathrm{E2E} = \mathrm{TTFT} + \mathrm{generation\_ms}$,
a~różnice Q4/Q8 ginęły w~budżecie prefillu.

Rysunek~\ref{fig:cold_dekompozycja_e2e} pokazuje dekompozycję E2E
w~protokole cold-cache ($n\approx 100$ per komórka model$\times$wariant).
...
\begin{figure}[H]
  ...
  \caption{Dekompozycja E2E na TTFT i~\emph{generation\_ms} (cold cache,
           ...
           $n\approx 100$. Prefill dominuje budżet czasu.}
  \label{fig:cold_dekompozycja_e2e}
\end{figure}

Na wykresie widać, że TTFT stanowi większość budżetu czasu i~prawie
nie zależy od wariantu kwantyzacji Q4. Dla \texttt{gemma3-4b/Q4\_K\_M}
mediana LN wynosi ok.~31{,}3~s TTFT przy 5{,}8~s generacji i~37{,}9~s E2E
--- prefill to ok.~83\% całkowitego opóźnienia. Warianty Q4\_0 i~Q4\_K\_M
mają TTFT 33{,}0~s oraz 31{,}3~s, a~generację 5{,}0~s oraz 5{,}8~s;
różnica kwantyzacji jest rzędu sekundy przy trzydziestosekundowym
prefillu. Wariant Q8\_0 ma TTFT ok.~61{,}3~s, co wynika z~większego
pliku GGUF przy zimnym ładowaniu wag do RAM, a~nie z~wolniejszego
dekodowania.

Taki rozkład jest konsekwencją architektury Raspberry~Pi~5.
Urządzenie nie dysponuje GPU przeznaczonym do inferencji LLM ---
całość obliczeń wykonuje CPU ARM Cortex-A76. Oficjalna dokumentacja
procesora BCM2712 podaje 32-bitowy interfejs LPDDR4X o~przepustowości
do 17~GB/s \cite{RaspberryPiBCM2712}. Prefill wymaga jednorazowego
przetworzenia całego promptu i~zbudowania KV-cache \cite{Pope2022};
przy zimnym starcie dodatkowo stronicuje wagi z~dysku do RAM
(\texttt{mmap} pliku GGUF). Faza dekodowania, w~której przy każdym
...
precyzji bitowej pozostaje niewidoczny.

Aby wyizolować czas generacji, zastosowano protokół \emph{warm-cache}:
stały \emph{system\_prompt}, \texttt{keep\_alive=30m},
\texttt{streaming=True} oraz
\emph{generation\_ms} $= \mathrm{e2e\_ms} - \mathrm{ttft\_ms}$
per rekord. Model i~KV-cache pozostają wówczas w~RAM między zapytaniami,
a~prefill ogranicza się do nowego sufiksu. Zysk zależy jednak od
konstrukcji promptu: KV-cache jest trafiany tylko wtedy, gdy tokeny
od początku wiadomości są identyczne. Szerszy opis tego mechanizmu
...
Rysunek~\ref{fig:dekompozycja_e2e} pokazuje tę samą dekompozycję
po przejściu na protokół warm-cache.
...
Zmiana protokołu daje czytelną poprawę. Dla \texttt{gemma3-4b/Q4\_K\_M}
TTFT spada z~ok.~31{,}3~s do ok.~2{,}44~s (redukcja rzędu 92\%),
a~E2E z~ok.~37{,}9~s do ok.~9{,}0~s. Udział generacji w~E2E rośnie
z~ok.~15\% do ok.~71\%, więc kwantyzacja przestaje być maskowana
przez prefill: Q4\_K\_M osiąga 6{,}42~s generacji wobec 9{,}66~s
dla Q8\_0, czyli przyspieszenie $1{,}50\times$. Dalsza analiza
czasów generacji --- model log-normalny i~zestawienie speedupów ---
jest przedmiotem kolejnych podrozdziałów.
