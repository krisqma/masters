...
i~\emph{prefix cachingiem}. Osiąga ona E2E na poziomie ok.~9~s
(tabela~\ref{tab:rekomendacja_produkcyjna}) przy wskaźniku halucynacji
1{,}17\% --- czternastokrotnie niższym niż druga najlepsza opcja (16{,}5\%,
tabela~\ref{tab:wyniki_finalne}).
...
\begin{itemize}
  \item \textbf{Krótki czas odpowiedzi:} założenie spełnione. Dzięki kwantyzacji
        ...
        caching}) osiągnięto E2E na poziomie ok.~9~s (TTFT ok.~2{,}4~s,
        generacja ok.~6{,}4~s) --- wartość akceptowalną dla asystenta głosowego,
...
\label{subsec:plan_rozwoju}

Wyniki przeprowadzonych badań wyznaczają kilka naturalnych kierunków dalszego
rozwoju, z~których najważniejszy dotyczy wag modelu, a~nie kolejnego strojenia
inferencji.

Analiza Optuny pokazała, że wariancję \emph{composite score} tłumaczy niemal
w~całości flaga halucynacji, podczas gdy \emph{nonhall\_score} pozostaje
w~wąskim przedziale niezależnie od dobranych hiperparametrów. Kwantyzacja
przyspiesza generację, lecz nie zmienia semantyki odpowiedzi, a~\emph{prefix
caching} skraca TTFT, nie wpływając na to, czy model zmyśla fakty przy
brakujących lub nieaktualnych odczytach. Halucynacje w~kategoriach
\texttt{missing\_data} i~\texttt{stale\_data} są zatem własnością wag, a~nie
konfiguracji uruchomieniowej. Jedyną dźwignią, która może je ograniczyć,
pozostaje dostrojenie modelu na przykładach docelowego zachowania.

Planowany etap zakłada fine-tuning rekomendowanego \texttt{gemma3:4b} metodą
LoRA, zrealizowany biblioteką \texttt{mlx-lm} na komputerze MacBook~M4
z~układem Apple Silicon. Zbiór treningowy ma liczyć ok.~400~przykładów
w~formacie \texttt{messages} (role \texttt{system}, \texttt{user}
i~\texttt{assistant}) i~pokrywać kategorie złotego zbioru testowego:
\texttt{factual\_status}, \texttt{missing\_data}, \texttt{conflicting\_data},
\texttt{stale\_data} oraz \texttt{paraphrase}. Celem eksperymentu jest
sprawdzenie, czy model dostrojony --- pozbawiony jawnych reguł w~prompcie
systemowym --- przewyższa model bazowy wyposażony w~te reguły właśnie
na najtrudniejszych scenariuszach, w~których instrukcje promptowe okazały
się mieć wpływ marginalny. Takie badanie wykracza poza ramy czasowe
niniejszej pracy; obecne wyniki stanowią dla niego empiryczne uzasadnienie.

Osobnym kierunkiem jest domknięcie zastrzeżenia metodologicznego dotyczącego
redukcji TTFT. Bezpośredni eksperyment \emph{cold} kontra \emph{warm cache}
dla identycznego promptu produkcyjnego pozwoliłby precyzyjnie skwantyfikować
zysk z~\emph{prefix cachingu} bez porównywania dwóch różnych protokołów
pomiarowych.

Wdrożenie w~docelowym środowisku Ośrodka Wypoczynkowego Politechniki
Warszawskiej w~Wildze (projekt Wilga~MiLL) wymaga natomiast zastąpienia
symulacji odczytów z~pliku CSV strumieniem danych na żywo (\emph{live feed})
z~fizycznych czujników oraz integracji z~istniejącą infrastrukturą IoT.

Dalsze prace mogą objąć rozszerzenie puli badanych modeli i~wariantów
kwantyzacji, ewaluację prototypu z~udziałem docelowych użytkowników
nietechnicznych oraz optymalizację warstw STT, TTS i~VAD pod kątem
opóźnienia i~naturalności interakcji głosowej.
