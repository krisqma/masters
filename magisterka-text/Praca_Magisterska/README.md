...
```bash
# sprawdź, czy lokalnie są dostępne narzędzia LaTeX
make doctor

# kompiluj PDF na bieżąco po każdym zapisie pliku
make watch

# zrób jednorazowy eksport PDF przez pdfLaTeX
make pdf

# zbuduj wszystkie warianty PDF: pdfLaTeX, LuaLaTeX i XeLaTeX
make all

# usuń pliki builda i artefakty pomocnicze
make clean
```

Domyślny plik podczas pracy na bieżąco znajduje się tutaj:

```text
build/pdflatex/main.pdf
```

Eksport po `make pdf` jest kopiowany tutaj:

```text
build/pdfs/main-pdflatex.pdf
```
...
### Auto-build i preview lokalnie
Najwygodniejsza lokalna opcja to `latexmk`, bo potrafi śledzić zmiany w plikach i przebudowywać PDF po każdym zapisie.

Po instalacji TeX Live / MacTeX można uruchomić:

```bash
make watch
```

To uruchamia ciągłą kompilację `pdfLaTeX` i zapisuje podgląd PDF do:

```text
build/pdflatex/main.pdf
```

Dostępne są też warianty:
...
Najwygodniejszy preview:
- na macOS otwórz `build/pdflatex/main.pdf` w `Preview` - plik zwykle odświeża się automatycznie po każdej przebudowie;
- w VS Code zainstaluj rozszerzenie `LaTeX Workshop` - repo zawiera gotowe ustawienia w `.vscode/settings.json`, więc PDF buduje się automatycznie przy zapisie i otwiera w zakładce edytora.

Uwaga: do działania potrzebny jest zainstalowany `latexmk`. Jeśli `latexmk -v` zwraca błąd, doinstaluj pełny pakiet TeX Live / MacTeX.
